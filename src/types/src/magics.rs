...
pub const fn get_magic_index(magics: &[BlackMagicEntry; Square::NUM], index_bits: usize, blockers: BitBoard, square: Square) -> usize {
    let magic = &magics[square as usize];
    let relevant_blockers = blockers.0 | magic.mask.0;
    let hash = relevant_blockers.wrapping_mul(magic.magic.0);
